...
class RateLimiter:
    ...
    def is_allowed(self, user_id: int) -> Tuple[bool, int]:
        ...
        self._requests[user_id] = [
            req_time for req_time in self._requests[user_id] if req_time > window_start
        ]
    ...
    def get_stats(self, user_id: int) -> Dict:
        ...
        recent = [
            req_time
            for req_time in self._requests.get(user_id, [])
            if req_time > window_start
        ]
        return {
            "user_id": user_id,
            "requests_in_window": len(recent),
            "limit": self.cfg.rate_limit.requests_per_minute,
            "window_seconds": self.cfg.rate_limit.seconds,
            "remaining": max(0, self.cfg.rate_limit.requests_per_minute - len(recent)),
        }
